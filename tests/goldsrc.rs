...
#[test]
fn a2s_info_old() {
    let query = ValveQuery::<GoldsrcParser>::bind("0.0.0.0:27415".parse().unwrap()).unwrap();
...
#[test]
fn a2s_player() {
    let query = ValveQuery::<GoldsrcParser>::bind("0.0.0.0:27515".parse().unwrap()).unwrap();
    query.set_timeout(Some(Duration::new(10, 0))).unwrap();
    query.connect(ADDR.parse().unwrap()).unwrap();
...
#[test]
fn a2s_rules() {
    let query = ValveQuery::<GoldsrcParser>::bind("0.0.0.0:27615".parse().unwrap()).unwrap();
    query.set_timeout(Some(Duration::new(10, 0))).unwrap();
    query.connect(ADDR.parse().unwrap()).unwrap();
